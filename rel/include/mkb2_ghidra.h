...
struct SeesawState {
  float angle; /* Current rotation angle as a float, in range [-0x8000, 0x7FFF]
                  similar to s16 rotation angles */
  float prev_angle;
  float angle_vel;
...
struct CmEntry { /* Challenge Mode Entry, see cmEntryFormat.txt by TwixNinja in
                    notes for more info */
  CmEntryType type;
  u8 arg;
...
struct GoalTape { /* Extra goaltape-specific data pointed to by goaltape
                     StageObjects */
...
enum { /* Sourced from 0x80370244 in Ghidra - I'm guessing SMD stands for Sub
          MoDe here? / Also I added SMD_INVALID, it's `-1` -Crafted */
       SMD_ADV_TOP = 0,
...
enum { LOD_HIGH = 0, LOD_MIDDLE = 1, LOD_LOW = 2, LOD_LOWEST = 3 };
...
struct TplBuffer { /* Buffer allocated for TPL files (with 32 extra bytes at the
                      beginning compared to the on-disc TPL file). Amusement
                      Vision TPL is different than standard Gamecube TPL */
  s32 texture_count;
...
struct GmaBuffer { /* Represents the first 32 bytes of buffer allocated for
                      loaded GMA files. The first 32 bytes are extra; not part
                      of the original GMA file */
  s32 model_count;
...
struct GmaModelHeader { /* Also known as a GCMF (GameCube Model Format?) */
  ...
  u16 translucid_material_count; /* In F-Zero GX this is "translucid material
                                    count" */
  s8 transform_mtx_count;
  undefined field_0x1f[0x1];
  s32 model_header_size; /* Memory size of this structure in bytes, including
                            texture description array and transform matrices. */
  struct GXTexObj
      *texobjs; /* Array of texobjs, one for each texture in the model */
  s8 default_mtx_indices[8]; /* Default (root?) indices into Transform Matrix
                                array */
...
struct Ape {
  ...
  void *game_mal; /* It's not, a lot of these labels were confusingly wrong?? -
                     bomb Is this even a float? -Crafted */
  ...
  short field53_0x84;
  u8 chara_anim_type; /* Some value that changes the type of animation the
                         character is doing. From standing still, to walking, to
                         the "I lost" state, etc */
  byte g_anim_step;
...
struct Camera {
  ...
  struct Vec3f pivot; /* Called 'intr' in the debug menu. The point which the
                         camera looks at - close to the monkey in normal play,
                         but can also be modified with the C-stick in test
                         camera. Camera pose is actually controled by pos and
                         rot - pivot is an intermediate value */
  struct Vec3s rot; /* Rotation of the camera. Called 'ang' in the debug menu */
  u8 mode; /* One byte representing camera mode (1 (dec) = SMB 1 style camera,
              75 (dec) = SMB 2 style camera, and there's a lot more (everything
              from 1 to about 105 - a few are duplicates. Each value corresponds
              to a different function in the camera function table */
  ...
  u16 next_fov; /* Next field of view - when the camera FOV changes through some
                   event (goal replay, new level start), this value is taken and
                   used for the FOV */
  ...
  u16 g_some_short;
  struct Vec3f
      g_initial_pivot; /* The pivot is set to this point at the beginning of the
                          spin-in sequence, and approaches g_dest_intr */
  ...
  struct Vec3f g_final_pivot; /* The pivot moves towards this point, and reaches
                                 it at the end of the spin-in sequence */
  ...
  struct Vec3f
      g_pos_desire; /* Something to do with camera rotation interpolation? */
  s16 g_pivot_x_rot;
...
struct Sprite {
  ...
  Font8
      g_probably_not_font; /* Is this actually a font? Or is it some kind of ID?
                              On the pause menu sprite, the monkey head won't
                              track the menu selection unless it's "4" */
  s16 g_counter; /* At least in the press start/select text sprites, this is
                    used as some kind of counter when ticking */
  s16 field10_0x12;
  undefined field_0x14[0xc];
  float field23_0x20;
  ...
  float g_lerp_value; /* fpara1? At least in the SCORE sprite, this is the
                         visual displayed score which counts up towards the
                         actual score */
  ...
  float field67_0x98;
  float field68_0x9c;
  char text[48]; /* If this sprite displays text, this is what it shows,
                    otherwise this is usually just an identifier name */
} __attribute__((__packed__));
...
struct DVDFileInfo { /* Not entirely sure about this one... I've filled in some
                        fields that only exist within `#ifdef MACOS` blocks
                        according to the GC SDK on my end. Leaving them out
                        seems wrong if I look at DVDFastOpen. Also I haven't put
                        *all* MACOS fields in there since I don't know the size
                        of IOParam (from a Mac header). */
...
typedef struct GSomethingWithPadMotorsStruct GSomethingWithPadMotorsStruct,
    *PGSomethingWithPadMotorsStruct;
...
struct SpriteDrawRequest { /* Used by Sprite disp() functions to render a
                              texture on the screen, sometimes multiple times
                              per disp() call to render multiple things
                              on-screen per Sprite object */
  undefined field_0x0[0x8];
  float field8_0x8;
  float field9_0xc;
  float field10_0x10;
...
struct CmStage { /* Stage in Challenge Mode. Stage course number is what's shown
                    in bottom left of screen */
  s32 stage_course_num;
  s32 stage_id;
} __attribute__((__packed__));

struct CmPlayerProgress { /* Seems to be one of these per player, not sure what
                             they are exactly yet */
  struct CmStage curr_stage;
  struct CmStage
      next_stages[3]; /* Appears to track the "next" stage for the player in the
                         first element (assuming a blue goal is taken). The
                         latter two elements are never updated, and I don't
                         think the first element is even used for anything */
  s16 field2_0x20;
  s16 g_next_stage_idx;
} __attribute__((__packed__));

enum { /* These are normally just #defines in the SDK's PAD library. Also these
...
struct GoalBag { /* Extra goalbag-specific state pointed to by goalbag
                    StageObjects */
...
struct PhysicsBall { /* A representation of a Ball with just the
                        physics/collision-related info */
  ...
  dword field25_0x54;
  float field26_0x58;
  dword itemgroup_idx; /* The itemgroup that this PhysicsBall is relative to,
                          aka in the local space of */
} __attribute__((__packed__));

struct Stobj { /* A "stage object" which is one of a: bumper, jamabar, goaltape,
                  party ball, wormhole, and others. */
  word idx;
  short id;
  StobjType type;
  ...
  void *extra_data; /* Extra stobj-type-specific data, such as switch stagedef
                       header for switches or goaltape struct for goaltapes.
                       Maybe worth making a union */
...
struct SeesawInfo { /* Allocated on the heap for an itemgroup if it's a seesaw.
                       Points to another struct also allocated on the heap that
                       contains physics state info like current rotation angle.
                     */
  short itemgroup_idx;
  word g_type; /* Seems to always be zero, but the way it's set seems to
                  indicate there may have been more than one type of seesaw at
                  some point */
...
struct Item { /* Represents an item that can be picked up by the player. These
                 are bananas in main game, but can be other pickups in the party
                 games. Are "tickable" like events, effects, etc. */
  word index;
  short id;
  ItemType type;
  s16 coin_type;
  ...
  undefined field_0x56[0x2];
  void (*item_coli_func)(struct Item *,
                         struct PhysicsBall *); /* Created by retype action */
  u8 itemgroup_idx;
...
struct UnlockInfo { /* Info about which stuff in the game is unlocked, such as
                       master, number of monkeys, party games, etc. Which story
                       mode / challenge mode stages are unlocked are stored
                       separately however. */
  byte master_unlocked;
  byte monkeys; /* Number of monkeys (lives) unlocked, aka the max you can use
                   in challenge mode */
  undefined field_0x2[0x1];
  byte staff_credits_game_unlocked;
  dword play_points; /* Seems to function as "current number of play points"
                        when you don't have everything unlocked, and "highest
                        play points record" when you do */
  dword newest_play_point_record;
  undefined field_0xc[0x2];
  word movies;       /* One bit for each of the 12 movies */
  dword party_games; /* Bitfield; normally 0x0001b600 when all are unlocked, and
                        not zero when only base games are unlocked */
  word g_movies_watched; /* Determines whether unlocked movies are grayed out;
                            does this mean "movie watched"? */
...
struct RaycastHit {
  u16 geom_flags; /* Bit 0 is set if the hit is valid. Also OR-d with some flags
                     from the tri/sphere/whatever the line trace hit */
  undefined field_0x2[0x2];
  struct Vec3f pos; /* Position of ray-geometry intersection */
  struct Vec3f
      normal; /* Geometry normal at point of ray-geometry intersection */
} __attribute__((__packed__));
...
struct Ball {
  ...
  PhysicsMode phys_mode; /* Something to do with physics state (Crashes the game
                            if set wrong) (8 bit bitmask) */
  struct Vec3f pos;
  struct Vec3f prev_pos;
  struct Vec3f vel; /* Velocity/speed */
  struct Vec3s g_some_rot;
  u8 idx; /* The index of the ball in the ball pool, aka 0 for the first ball, 1
             for the second */
  ...
  s16 g_monkey_angle; /* Seems to be related to the direction the monkey is
                         facing */
  undefined field_0x94[0x4];
  u32 g_effect_flags;
  BallPhysFlags
      phys_flags; /* Some more flags related to ball state? The lowest-order bit
                     may represent "is ball touching the ground" and I believe
                     if affects the physics */
  ...
  struct Ape *ape;
  int field50_0x108;
  struct Vec3f ape_facedir_point; /* The point of interest that the monkey looks
                                     at (goal, banana, etc) */
  float something_with_ape_facedir; /* Approaches 1 the closer you are to the
                                       point of interest */
  struct Vec3f g_last_collision_normal; /* Maybe inverse of the normal of the
                                           last triangle collided with? */
  ...
  u8 g_ball_color_index; /* Ball color by player index: 0 red, 1 blue, 2 yellow,
                            3 green, etc */
  ...
  u8 g_some_game_flag; /* Some values make the ball return to the start position
                        */
  u8 g_some_model_flag; /* Changes the ball model to lower poly variants, also
                           apparantly the Dole blimp? */
  undefined field_0x17a[0x2];
  struct RaycastHit raycast_down_hit; /* Result of raycast downward from ball's
                                         origin with the stage */
...
struct Itemgroup { /* Contains the current animation-related state of each item
                      group in a stage (each thing corresponding to a collision
                      header in the stagedef) */
  dword playback_state; /* Corresponding to the switch playback type which is
                           controlling the item group, see PlaybackState */
  dword anim_frame;
  struct Vec3f position;
...
enum { GX_DF_NONE = 0, GX_DF_SIGN = 1, GX_DF_CLAMP = 2 };
...
struct GraphicsInfo { /* Is this the right size for the struct? maybe? -Crafted
                       */
  void *
      active_framebuffer; /* A pointer to either framebuffer1 or framebuffer2 */
  void *framebuffer1;
  void *framebuffer2;
  u32 frame_count;
  u32 active_framebuffer_index; /* Either 0 or 1, representing framebuffer1 or
                                   framebuffer2 */
...
struct MemCardInfo { /* Some struct that seems to hold per-memcard info; there
                        are two statically allocated, one for each memory card
                        slot (I think) */
...
struct PadStatusGroup { /* A set of PADStatus structs for a given controller,
                           with each representing a different "filtered" version
                           of the inputs */
  struct PADStatus
      raw; /* The raw PADStatus read by PADRead() for the controller */
  struct PADStatus prev_tick; /* The PADStatus from the previous frame */
  struct PADStatus pressed;   /* PADStatus representing the digital inputs that
                                 were just pressed this frame */
  struct PADStatus released;  /* PADStatus representing the digital inputs that
                                 were released this frame */
  struct PADStatus
      repeated; /* PADStatus of digital inputs that "repeat" similar to holding
                   a key down on a keyboard: bit is high on first press, then
                   there's a pause of many frames, then the bit is high every 4
                   frames */
} __attribute__((__packed__));

typedef struct DigitalInputGroup DigitalInputGroup, *PDigitalInputGroup;

struct DigitalInputGroup { /* Consolidated bitfields for digital button inputs
                              corresponding to the five PadStatusGroup types */
  PadDigitalInput raw;
...
struct AnalogInputGroup { /* Consolidated thresholded analog input bitfields
                             corresponding to the five groups of PadStatusGroup
                           */
  PadAnalogInput raw;
...
struct ModeInfo { /* I don't know what to call this, but there's some important
                     global game info in here! -Complex */
  BallMode ball_mode; /* Correlates with the ball's 'mode' in the debug menu's
                         ball display. Bonus stages have 0x40 set, final stages
                         in a difficulty have 0x1000 set.  0x8 seems to stop the
                         timer? -Crafted */
  ...
  undefined2 cm_course_stage_num; /* Current course stage num, updated
                                     immediately after completing stage */
  ...
  s16 cm_stage_id; /* Current challenge mode stage id, updated immediately after
                      finishing stage */
...
struct Event { /* A subset of continuously running game functionality that can
                  be turned on or off independent of other Events */
  Status status;
  undefined padding_0x1[0x3];
  char *name;              /* The name of the event */
  void (*init_func)(void); /* Normally, called once when status is STAT_INIT,
                              then status is set to STAT_NORMAL */
  void (*tick_func)(
      void); /* Normally, called every frame if status is STAT_NORMAL */
  void (*dest_func)(
      void); /* "destruct:" or "destroy" function; normally, called once when
                status is STAT_DEST, then status is set to STAT_NULL */
  u32 tick_time; /* For performance metrics (presumably this is what's seen in
                    the debug overlay performance > event menu) */
} __attribute__((__packed__));
...
enum { BANANA_SINGLE = 0, BANANA_BUNCH = 1 };
...
struct OptiGXSettings { /* Opti = For optimization - I don't actually know how
                           big this struct is -Crafted */
...
enum { OF_G_SMTH_WITH_CAMERA = 2, OF_GAME_PAUSED = 8 };
...
struct DipSwitchesOld { /* Unused struct - maybe I'll use it once Ghidra
                           supports bitfields a bit nicer in the decompilation
                         */
...
struct PoolInfo { /* Metadata and status info for lists of "tickable" objects
                     like the sprite, effect, stobj, and item lists */
  dword len; /* The size of the object pool, aka the maximum number of non-empty
                objects the pool can hold */
  dword low_free_idx; /* The last known "low index" of an empty object in the
                         pool. Reset at the start of the frame to the lowest
                         free idx in the list, and increments as objects are
                         allocated */
  dword upper_bound;  /* Number of objects from the start of the list until the
                         last non-empty object. Usually the list is iterated over
                         from 0 to this value, checking each tickable if it's
                         non-empty. Reset at the start of each frame to one past
                         the last non-empty object in the pool, and increased if
                         the low idx bumps into it */
  u8 *status_list;    /* Byte array of same length as the pool, each byte
                         corresponds to some status of the corresponding object.
                         Usually either 0 for "empty slot" and either 1 or 2 for
                         "active". This does not appear to be a Status like Events
                         use. */
} __attribute__((__packed__));
...
typedef struct StagedefBackgroundAnimHeader StagedefBackgroundAnimHeader,
    *PStagedefBackgroundAnimHeader;

typedef struct StagedefAnimKeyframe StagedefAnimKeyframe,
    *PStagedefAnimKeyframe;

enum { EASING_CONSTANT = 0, EASING_LINEAR = 1, EASING_SMOOTH = 2 };
...
typedef struct StagedefEffectHeader StagedefEffectHeader,
    *PStagedefEffectHeader;
...
typedef struct StagedefTextureScroll StagedefTextureScroll,
    *PStagedefTextureScroll;
...
struct StagedefEffect1 { /* Likely used for animated textures, such as the
                            silhouettes in Night */
  dword field0_0x0;
  dword field1_0x4;
  dword field2_0x8;
  word field3_0xc;
...
typedef struct StagedefFogAnimHeader StagedefFogAnimHeader,
    *PStagedefFogAnimHeader;
...
typedef struct StagedefColiCylinder StagedefColiCylinder,
    *PStagedefColiCylinder;

typedef struct StagedefFalloutVolume StagedefFalloutVolume,
    *PStagedefFalloutVolume;

typedef struct StagedefBackgroundModel StagedefBackgroundModel,
    *PStagedefBackgroundModel;

typedef struct StagedefForegroundModel StagedefForegroundModel,
    *PStagedefForegroundModel;

typedef struct StagedefReflectiveStageModel StagedefReflectiveStageModel,
    *PStagedefReflectiveStageModel;

typedef struct StagedefStageModelInstance StagedefStageModelInstance,
    *PStagedefStageModelInstance;

typedef struct StagedefStageModelPtrA StagedefStageModelPtrA,
    *PStagedefStageModelPtrA;

typedef struct StagedefStageModelPtrB StagedefStageModelPtrB,
    *PStagedefStageModelPtrB;
...
typedef struct StagedefDynamicReflectionPlane StagedefDynamicReflectionPlane,
    *PStagedefDynamicReflectionPlane;

typedef struct StagedefMystery3 StagedefMystery3, *PStagedefMystery3;

enum { ANIM_LOOP = 0, ANIM_PLAY_ONCE = 1, ANIM_SEESAW = 2 };
...
typedef struct StagedefBackgroundAnim2Header StagedefBackgroundAnim2Header,
    *PStagedefBackgroundAnim2Header;
...
struct StagedefForegroundModel {
  undefined4 field0_0x0; /* Unknown - typically 0x0000001F, sometimes 0x00000007
                            or 0x0000000F */
  char *model_name;
...
struct StagedefColiHeader {
  ...
  u16 anim_group_id; /* For use with buttons - Must be non-zero if you want to
                        assign a group ID */
  undefined2 g_not_padding;
  u32 button_count;
  struct StagedefButton *button_list;
  ...
  float seesaw_sensitivity; /* Higher is more sensitive, negative makes the
                               seesaw reversed */
  float seesaw_friction;    /* Lower is looser */
  float seesaw_spring;      /* 0 prevents the seesaw from resetting */
  u32 wormhole_count;
  struct StagedefWormhole *wormhole_list;
  u32 initial_playback_state; /* Should this be split into 2x padding bytes +
                                 PlaybackState enum? */
...
typedef struct GmaVertexControlHeader GmaVertexControlHeader,
    *PGmaVertexControlHeader;

struct GmaVertexControlHeader { /* Also called "Model Type 1". This structure
                                   appears to outline various information
                                   relating to skinned and effective models
                                   only. A such, it is assumed that it is data
                                   for controlling the vertices. */
  s32 vertex_count;
...
struct GmaHeader { /* First 8 bytes of a GMA file. Following this is an
                      arbitrary-length list of 8-byte GMA model entries */
  s32 model_count; /* The number of GCMF models in the file (including null
                      entries) */
  s32 header_size; /* Size of header including FIFO padding before GCMF models
                      begin. Also called "model base position" */
} __attribute__((__packed__));
...
typedef struct GmaTextureDescriptor GmaTextureDescriptor,
    *PGmaTextureDescriptor;
...
enum { GX_ANISO_1 = 0, GX_ANISO_2 = 1, GX_ANISO_4 = 2, GX_MAX_ANISOTROPY = 3 };
...
struct GmaTextureDescriptor {
  ...
  byte unk_0xC;
  byte g_is_swappable_texture; /* Boolean that indicates this texture may be
                                  swapped out at runtime. This is used for
                                  lap-related textures in F-Zero GX. */
  s16 tex_descriptor_idx; /* Texture descriptor index, matches its zero-indexed
                             value in this array */
  undefined field_0x10[0x10];
} __attribute__((__packed__));
...
struct OSContext {
  ...
  u32 gqr[8]; /* Place Gekko regs at the end so we have minimal changes to
                 existing code */
  f64 psf[32];
} __attribute__((__packed__));
...
struct OSThread {
  ...
  struct OSThreadQueue
      *queueJoin;        /* list of threads waiting for termination (join) */
...
enum { GX_NONE = 0, GX_DIRECT = 1, GX_INDEX8 = 2, GX_INDEX16 = 3 };
typedef undefined4 GXAttrType;

enum { GX_VCACHE_MISS_ALL = 0, GX_VCACHE_MISS_POS = 1, GX_VCACHE_MISS_NRM = 2 };
...
enum { GX_READ_00 = 0, GX_READ_FF = 1, GX_READ_NONE = 2 };
typedef undefined4 GXAlphaReadMode;

typedef union PPCWGPipe PPCWGPipe, *PPPCWGPipe;

union PPCWGPipe { /* PPC Write Gather Pipe. Original field names didn't have v_
                     prefix */
  u8 v_u8;
  u16 v_u16;
  u32 v_u32;
...
enum { GX_CLR_RGB = 0, GX_CLR_RGBA = 1, GX_NRM_NBT3 = 2 };
...
enum { GX_TF_C4 = 8, GX_TF_C8 = 9, GX_TF_C14X2 = 10 };
...
enum { VI_XFBMODE_SF = 0, VI_XFBMODE_DF = 1 };
...
enum { GX_CLAMP = 0, GX_REPEAT = 1, GX_MIRROR = 2, GX_MAX_TEXWRAPMODE = 3 };
...
enum { GX_CLAMP_NONE = 0, GX_CLAMP_TOP = 1, GX_CLAMP_BOTTOM = 2 };
...
enum { GX_ZT_DISABLE = 0, GX_ZT_ADD = 1, GX_ZT_REPLACE = 2, GX_MAX_ZTEXOP = 3 };
...
struct ChunkInfo { /* Header data for a heap chunk. The first 32 bytes of a heap
                      chunk is dedicated to this metadata and padding, and the
                      rest is the actual data in the chunk */
  struct ChunkInfo *prev;
  struct ChunkInfo *next;
  u32 size;
  u8 padding[20]; /* This is to make the data portion of the chunk 32-bit
                     aligned */
} __attribute__((__packed__));
...
enum { GX_TL_IA8 = 0, GX_TL_RGB565 = 1, GX_TL_RGB5A3 = 2, GX_MAX_TLUTFMT = 3 };
...
enum { GX_GM_1_0 = 0, GX_GM_1_7 = 1, GX_GM_2_2 = 2 };
...
enum { GX_ZC_LINEAR = 0, GX_ZC_NEAR = 1, GX_ZC_MID = 2, GX_ZC_FAR = 3 };
...
enum { GX_CLIP_ENABLE = 0, GX_CLIP_DISABLE = 1 };
...
enum { GX_COPY_PROGRESSIVE = 0, GX_COPY_INTLC_EVEN = 2, GX_COPY_INTLC_ODD = 3 };
...
enum { GX_XF_FLUSH_NONE = 0, GX_XF_FLUSH_SAFE = 8 };
...
enum { GX_MT_NULL = 0, GX_MT_XF_FLUSH = 1, GX_MT_DL_SAVE_CONTEXT = 2 };
...
/* Attempting output of invalid function pointer type declaration: CameraFunc
 * *[93] * */
extern void (*camera_funcs)(struct Camera *, struct Ball *);
...
void load_main_loop_rel_and_run(char *rel_filepath,
                                struct RelBufferInfo *buffer_info);
...
void GXSetBlendMode_cached(GXBlendMode type, GXBlendFactor src_factor,
                           GXBlendFactor dst_factor, GXLogicOp op);
void GXSetTevSwapModeTable_cached(GXTevSwapSel id, GXTevColorChan red,
                                  GXTevColorChan green, GXTevColorChan blue,
                                  GXTevColorChan alpha);
void g_GXSetTevSwapModeTable_wrapper(GXTevSwapSel param_1,
                                     GXTevColorChan param_2,
                                     GXTevColorChan param_3,
                                     GXTevColorChan param_4,
                                     GXTevColorChan param_5);
void GXSetTevSwapMode_cached(GXTevStageID stage, GXTevSwapSel ras_sel,
                             GXTevSwapSel tex_sel);
void GXSetColorUpdate_cached(GXBool update_enable);
void GXSetZCompLoc_cached(GXBool before_tex);
void GXSetTevColorIn_cached(GXTevStageID stage, GXTevColorArg a,
                            GXTevColorArg b, GXTevColorArg c, GXTevColorArg d);
void GXSetTevAlphaIn_cached(GXTevStageID stage, GXTevAlphaArg a,
                            GXTevAlphaArg b, GXTevAlphaArg c, GXTevAlphaArg d);
void GXSetTevColorOp_cached(GXTevStageID stage, GXTevOp op, GXTevBias bias,
                            GXTevScale scale, GXBool clamp, GXTevRegID out_reg);
void GXSetTevAlphaOp_cached(GXTevStageID stage, GXTevOp op, GXTevBias bias,
                            GXTevScale scale, GXBool clamp, GXTevRegID out_reg);
void g_GXSetTevColorIn_GXSetTevAlphaIn_wrapper(GXTevStageID param_1,
                                               int param_2);
void g_GXSetTevOrder_wrapper(GXTevStageID param_1, GXTexCoordID param_2,
...
void GXSetIndTexOrder_cached(GXIndTexStageID ind_stage, GXTexCoordID tex_coord,
                             GXTexMapID tex_map);
void g_GXSetIndTexMtx_wrapper(GXIndTexMtxID param_1, float param_2[2][3],
                              char param_3);
void GXSetTevIndirect_cached(GXTevStageID tev_stage, GXIndTexStageID ind_stage,
                             GXIndTexFormat format, GXIndTexBiasSel bias_sel,
                             GXIndTexMtxID matrix_sel, GXIndTexWrap wrap_s,
                             GXIndTexWrap wrap_t, GXBool add_prev,
                             GXBool utc_lod, GXIndTexAlphaSel alpha_sel);
...
void GXSetTexCoordGen2_cached(GXTexCoordID dst_coord, GXTexGenType func,
...
void opti_GXSetChanCtrl(GXChannelID chan, GXBool enable, GXColorSrc amb_src,
                        GXColorSrc mat_src, u32 light_mask, GXDiffuseFn diff_fn,
                        GXAttnFn attn_fn);
void GXSetZMode_cached(GXBool compare_enable, GXCompare func,
                       GXBool update_enable);
void g_read_something_from_prev_GX_settings(undefined *param_1,
                                            undefined4 *param_2,
                                            undefined *param_3);
void g_set_some_gx_settings(void);
void g_draw_something(double param_1, double param_2, double param_3,
                      ushort *param_4);
void mark_finished_waiting_for_dvdread(s32 result,
                                       struct DVDFileInfo *file_info);
u32 read_entire_file_using_dvdread_prio_async(struct DVDFileInfo *fileInfo,
...
void InsertAlarm(undefined4 *param_1, undefined4 param_2, uint param_3,
                 uint param_4, undefined4 param_5);
void OSSetAlarm(undefined4 *param_1, undefined4 param_2, undefined4 param_3,
                undefined4 param_4, undefined4 param_5);
void OSSetPeriodicAlarm(undefined4 *param_1, undefined4 param_2, int param_3,
                        uint param_4, undefined4 param_5, undefined4 param_6,
                        undefined4 param_7);
void OSCancelAlarm(int *param_1);
undefined8 DecrementerExceptionCallback(undefined4 param_1,
                                        struct OSContext *param_2);
ChunkInfo *DLInsert(struct ChunkInfo *free_chunk_list,
                    struct ChunkInfo *new_free_chunk);
...
void __OSSaveFPUContext(undefined8 param_1, undefined8 param_2,
...
void (*__OSGetInterruptHandler(__OSInterrupt interrupt))(__OSInterrupt,
                                                         struct OSContext *);
...
undefined8 ExternalInterruptHandler(undefined4 param_1, undefined4 *param_2,
...
undefined4 OSCreateThread(struct OSThread *thread, void *func, void *param,
                          void *stackBase, u32 stackSize, OSPriority priority,
                          u16 attribute);
...
void SIInit(void);
undefined4 __SITransfer(uint param_1, undefined4 *param_2, int param_3,
...
undefined4 SITransfer(uint param_1, undefined4 *param_2, int param_3,
                      undefined4 param_4, int param_5, int param_6,
...
undefined4 EXIImm(int param_1, byte *param_2, int param_3, int param_4,
                  undefined4 param_5);
undefined4 EXIImmEx(int param_1, byte *param_2, int param_3, int param_4);
undefined4 EXIDma(int param_1, uint param_2, undefined4 param_3, int param_4,
                  undefined4 param_5);
...
void C_MTXLookAt(Mtx *mtx, struct Vec3f *cam_pos, struct Vec3f *cam_up,
                 struct Vec3f *target);
void C_MTXFrustum(double param_1, double param_2, double param_3,
                  double param_4, double param_5, double param_6,
                  float *param_7);
void C_MTXPerspective(Mtx44 *m, double fovy, double aspect, double n, double f);
void C_MTXOrtho(double param_1, double param_2, double param_3, double param_4,
                double param_5, double param_6, float *m);
...
void SeekTwiceBeforeRead(undefined4 param_1, undefined4 param_2, uint param_3,
                         undefined4 param_4);
undefined4 DVDLowRead(undefined4 param_1, uint param_2, uint param_3,
                      undefined4 param_4);
...
undefined4 DVDLowAudioStream(uint param_1, undefined4 param_2, uint param_3,
                             undefined4 param_4);
undefined4 DVDLowRequestAudioStatus(uint param_1, undefined4 param_2);
undefined4 DVDLowAudioBufferConfig(int param_1, uint param_2,
                                   undefined4 param_3);
...
bool DVDConvertEntrynumToPath(s32 entry_num, char *out_path,
                              size_t max_path_size);
undefined4 DVDGetCurrentDir(int param_1, int param_2);
void DVDChangeDir(char *dirName);
BOOL32 DVDReadAsyncPrio(struct DVDFileInfo *fileInfo, void *addr, s32 length,
                        s32 offset, void (*callback)(s32, struct DVDFileInfo *),
                        s32 prio);
void cbForReadAsync(undefined4 param_1, int param_2);
s32 DVDReadPrio(struct DVDFileInfo *fileInfo, void *addr, s32 length,
                s32 offset, s32 prio);
...
void DVDPrepareStreamAsync(undefined4 *param_1, uint param_2, uint param_3,
                           undefined4 param_4);
void cbForReadAsync(undefined4 param_1, int param_2);
void DVDInit(void);
...
undefined4 DVDReadAbsAsyncPrio(undefined4 *param_1, undefined4 param_2,
                               undefined4 param_3, undefined4 param_4,
                               undefined4 param_5, int param_6);
undefined4 DVDReadAbsAsyncForBS(undefined4 *param_1, undefined4 param_2,
                                undefined4 param_3, undefined4 param_4,
                                undefined4 param_5);
undefined4 DVDReadDiskID(undefined4 *param_1, undefined4 param_2,
                         undefined4 param_3);
undefined4 DVDPrepareStreamAbsAsync(undefined4 *param_1, undefined4 param_2,
                                    undefined4 param_3, undefined4 param_4);
...
undefined4 DVDGetStreamErrorStatusAsync(undefined4 *param_1,
                                        undefined4 param_2);
undefined4 DVDGetStreamPlayAddrAsync(undefined4 *param_1, undefined4 param_2);
undefined4 DVDInquiryAsync(undefined4 *param_1, undefined4 param_2,
                           undefined4 param_3);
void DVDReset(void);
...
void setFbbRegs(int param_1, uint *param_2, uint *param_3, uint *param_4,
                uint *param_5);
void setVerticalRegs(uint param_1, ushort param_2, byte param_3, short param_4,
...
void ARStartDMA(int param_1, undefined4 param_2, undefined4 param_3,
                undefined4 param_4);
...
void DTKInit(void);
undefined4 DTKQueueTrack(char *param_1, undefined4 *param_2, undefined4 param_3,
                         undefined4 param_4);
...
undefined4 ReadArrayUnlock(int param_1, uint param_2, byte *param_3,
...
int __CARDRead(int param_1, undefined4 param_2, uint param_3,
               undefined4 param_4, undefined4 param_5);
void BlockWriteCallback(int param_1, int param_2);
int __CARDWrite(int param_1, undefined4 param_2, uint param_3,
                undefined4 param_4, undefined4 param_5);
...
void __CARDCheckSum(ushort *param_1, uint param_2, short *param_3,
                    short *param_4);
...
s32 CARDMountAsync(s32 chan, void *workArea, void *detachCallback,
                   void *attachCallback);
...
void CARDCreateAsync(s32 chan, char *fileName, u32 size,
                     struct CARDFileInfo *fileInfo, void *callback);
int __CARDSeek(int *param_1, int param_2, uint param_3, int **param_4);
void ReadCallback(int param_1, int param_2);
s32 CARDReadAsync(struct CARDFileInfo *fileInfo, void *buf, s32 length,
...
int CARDSetStatusAsync(int param_1, int param_2, int param_3,
                       undefined4 param_4);
int CARDRenameAsync(int param_1, char *param_2, char *param_3,
                    undefined4 param_4);
...
void GXInitFifoLimits(struct GXFifoObj *fifo, u32 hi_water_mark,
                      u32 lo_water_mark);
...
void GXAdjustForOverscan(struct GXRenderModeObj *rmin,
                         struct GXRenderModeObj *rmout, u16 hor, u16 ver);
...
void GXSetCopyFilter(GXBool aa, undefined sample_pattern, GXBool vf,
                     u8 vfilter[7]);
...
void GXInitLightSpot(struct GXLightObj *lt_obj, float cutoff,
                     GXSpotFn spot_func);
void GXInitLightDistAttn(struct GXLightObj *lt_obj, float ref_distance,
                         float ref_brightness, GXDistAttnFn dist_func);
...
void GXSetChanCtrl(GXChannelID chan, GXBool enable, GXColorSrc amb_src,
                   GXColorSrc mat_src, GXLightID light_mask,
                   GXDiffuseFn diff_fn, GXAttnFn attn_fn);
u32 GXGetTexBufferSize(u16 width, u16 height, u32 format, GXBool mipmap,
                       u8 max_lod);
void __GetImageTileCount(int param_1, ushort param_2, ushort param_3,
                         int *param_4, int *param_5, undefined4 *param_6);
void GXInitTexObj(struct GXTexObj *obj, void *image_ptr, u16 width, u16 height,
                  GXTexFmt format, GXTexWrapMode wrap_s, GXTexWrapMode wrap_t,
                  GXBool mipmap);
void GXInitTexObjLOD(struct GXTexObj *obj, GXTexFilter min_filt,
                     GXTexFilter mag_filt, float min_lod, float max_lod,
                     float lod_bias, GXBool bias_clamp, GXBool do_edge_lod,
                     GXAnisotropy max_aniso);
void GXInitTexObjData(struct GXTexObj *obj, void *image_ptr);
void GXInitTexObjWrapMode(struct GXTexObj *obj, GXTexWrapMode wrap_s,
                          GXTexWrapMode wrap_t);
void GXGetTexObjAll(struct GXTexObj *obj, void **image_ptr, u16 *width,
...
void g_GX_something(uint *param_1, uint *param_2, uint *param_3, float *param_4,
                    float *param_5, float *param_6, byte *param_7,
                    undefined *param_8, uint *param_9);
void GXLoadTexObjPreLoaded(struct GXTexObj *obj, struct GXTexRegion *region,
                           GXTexMapID id);
void GXLoadTexObj(struct GXTexObj *obj, GXTexMapID id);
void GXInitTexCacheRegion(struct GXTexRegion *region, GXBool is_32b_mipmap,
                          u32 tmem_even, GXTexCacheSize size_even, u32 tmem_odd,
                          GXTexCacheSize size_odd);
void GXInitTlutRegion(struct GXTlutRegion *region, u32 tmem_addr,
                      GXTlutSize tlut_size);
void GXInvalidateTexAll(void);
GXTexRegion *(*GXSetTexRegionCallback(GXTexRegion *(*f)(
    struct GXTexObj *, GXTexMapID)))(struct GXTexObj *, GXTexMapID);
...
void GXSetIndTexMtx(GXIndTexMtxID mtx_sel, float offset_mtx[2][3],
                    s8 scale_exp);
...
void GXSetTevSwapMode(GXTevStageID stage, GXTevSwapSel ras_sel,
                      GXTevSwapSel tex_sel);
void GXSetTevSwapModeTable(GXTevSwapSel id, GXTevColorChan red,
                           GXTevColorChan green, GXTevColorChan blue,
                           GXTevColorChan alpha);
...
void GXSetFog(GXFogType type, float startz, float endz, float nearz, float farz,
...
void GXDrawCubeFace(double param_1, double param_2, double param_3,
                    double param_4, double param_5, double param_6,
                    double param_7, double param_8, int param_4_00,
                    int param_5_00);
...
void GXSetViewportJitter(float xOrig, float yOrig, float wd, float ht,
                         float nearZ, float farZ, u32 field);
void GXSetViewport(float xOrig, float yOrig, float wd, float ht, float nearZ,
                   float farZ);
...
void GXReadXfRasMetric(undefined4 *param_1, undefined4 *param_2,
                       undefined4 *param_3, undefined4 *param_4);
undefined4 ARCInitHandle(void *arc_data, struct ARCHandle *out_arc_handle);
BOOL32 ARCOpen(struct ARCHandle *arc_handle, char *file,
               struct ArcFileInfo *arcFileInfo);
...
int PERFInit(int param_1, uint param_2, uint param_3, undefined *param_4,
             undefined4 param_5, undefined4 param_6);
...
void empty_function(void);
uint seqStartPlay(int *param_1, int *param_2, int param_3, int *param_4,
                  uint *param_5, undefined param_6, undefined2 param_7);
...
int *do_voice_portamento(byte param_1, char param_2, char param_3, int param_4,
                         undefined4 *param_5);
int *StartLayer(undefined2 param_1, int param_2, undefined4 param_3,
                undefined4 param_4, uint param_5, byte param_6, uint param_7,
                uint param_8, byte param_9, undefined param_10, ushort param_11,
                undefined2 param_12, int param_13, undefined param_14,
                undefined param_15, int param_16);
int *StartKeymap(undefined2 param_1, short param_2, undefined4 param_3,
                 undefined4 param_4, uint param_5, byte param_6, uint param_7,
                 uint param_8, byte param_9, undefined param_10,
                 ushort param_11, undefined2 param_12, uint param_13,
                 undefined param_14, undefined param_15, int param_16);
int *synthStartSound(uint param_1, int param_2, undefined4 param_3,
                     uint param_4, byte param_5, uint param_6, uint param_7,
                     uint param_8, undefined param_9, ushort param_10,
                     undefined2 param_11, undefined param_12, short param_13,
                     undefined param_14, int param_15);
...
int *synthFXStart(undefined2 param_1, byte param_2, uint param_3,
...
void synthVolume(uint param_1, uint param_2, uint param_3, undefined param_4,
                 undefined4 param_5);
...
undefined4 g_call_synthFXSetCtrl_with_irq_disabled(uint param_1, byte param_2,
                                                   byte param_3);
undefined4 g_call_synthFXSetCtrl14_with_irq_disabled(uint param_1, byte param_2,
                                                     uint param_3);
undefined4 sndFXKeyOff(uint param_1);
int *sndFXStartEx(undefined2 param_1, byte param_2, uint param_3, byte param_4);
int *sndFXStartParaInfo(undefined2 param_1, byte param_2, uint param_3,
                        byte param_4, byte *param_5);
...
void sndSetAuxProcessingCallbacks(uint param_1, int param_2, undefined4 param_3,
                                  char param_4, uint param_5, int param_6,
...
void sndStreamARAMUpdate(int param_1, uint param_2, uint param_3, uint param_4,
                         uint param_5);
void CheckOutputMode(undefined *param_1, undefined *param_2);
int sndStreamAllocEx(undefined param_1, undefined4 param_2, int param_3,
                     undefined4 param_4, undefined param_5, undefined param_6,
                     undefined param_7, undefined param_8, undefined param_9,
                     undefined param_10, uint param_11, undefined4 param_12,
                     undefined4 param_13, undefined2 *param_14);
...
undefined4 dataInsertLayer(ushort param_1, undefined4 param_2,
                           undefined2 param_3);
...
void SelectSource(int param_1, int param_2, uint *param_3, undefined4 param_4,
...
int *macStart(uint param_1, byte param_2, byte param_3, short param_4,
              byte param_5, undefined param_6, undefined param_7, uint param_8,
              byte param_9, undefined param_10, ushort param_11,
              undefined param_12, byte param_13, undefined param_14,
              undefined param_15, int param_16);
void macInit(void);
int vidInit(void);
...
undefined4 adsrHandleLowPrecision(char *param_1, undefined2 *param_2,
                                  short *param_3);
void vsInit(void);
...
void ScanIDListReverse(ushort *param_1, int *param_2, byte param_3,
                       int param_4);
undefined4 sndPushGroup(int *param_1, short param_2, undefined4 param_3,
                        int *param_4, int *param_5);
undefined4 sndPopGroup(void);
uint seqPlaySong(short param_1, short param_2, int *param_3, uint *param_4,
                 char param_5, undefined param_6);
void sndSeqPlayEx(short param_1, short param_2, int *param_3, uint *param_4,
                  undefined param_5);
...
undefined4 salCheckVolErrorAndResetDelta(undefined2 *param_1,
                                         undefined2 *param_2, short *param_3,
...
void salCalcVolume(double param_1, double param_2, double param_3, char param_4,
                   float *param_5, uint param_6, uint param_7, int param_8,
                   int param_9);
void UpdateRoomDistances(void);
void CheckRoomStatus(void);
void CheckDoorStatus(void);
void CalcEmitter(int param_1, float *param_2, float *param_3, float *param_4,
                 float *param_5, float *param_6);
void SetFXParameters(double param_1, double param_2, undefined8 param_3,
                     double param_4, double param_5, int param_6);
...
undefined4 AddStartingEmitter(double param_1, double param_2, double param_3,
                              double param_4, double param_5, int param_6);
void StartContinousEmitters(void);
void s3dHandle(void);
void s3dInit(uint param_1);
int sndInit(byte param_1, undefined param_2, undefined param_3, byte param_4,
...
void inpAddCtrl(int param_1, uint param_2, undefined4 param_3, byte param_4,
                int param_5);
...
int sndBSearch(undefined4 param_1, int param_2, int param_3, int param_4,
               undefined *param_5);
...
undefined4 hwInit(undefined4 *param_1, byte param_2, byte param_3,
                  uint param_4);
...
void hwInitSamplePlayback(int param_1, undefined2 param_2, undefined4 *param_3,
...
void hwSetVirtualSampleLoopBuffer(int param_1, undefined4 param_2,
                                  undefined4 param_3);
...
void hwSetVolume(double param_1, double param_2, double param_3, int param_4,
                 char param_5, uint param_6, uint param_7);
void hwOff(int param_1);
void hwSetAUXProcessingCallbacks(uint param_1, undefined4 param_2,
                                 undefined4 param_3, undefined4 param_4,
                                 undefined4 param_5);
int hwGetPos(int param_1);
void hwFlushStream(int param_1, uint param_2, int param_3, uint param_4,
                   undefined4 param_5, undefined4 param_6);
...
void aramUploadData(undefined4 param_1, undefined4 param_2, undefined4 param_3,
                    int param_4, undefined4 param_5, undefined4 param_6);
...
undefined4 ReverbHICreate(double param_1, double param_2, double param_3,
                          double param_4, double param_5, double param_6,
                          void *param_7);
undefined4 ReverbHIModify(double param_1, double param_2, double param_3,
                          double param_4, double param_5, double param_6,
                          void *param_7);
void DoCrossTalk(undefined8 param_1, undefined8 param_2, uint *param_3,
                 uint *param_4);
...
void empty_function(void);
undefined4 return_0(void);
void g_some_printf_function_0(undefined8 param_1, undefined8 param_2,
                              undefined8 param_3, undefined8 param_4,
                              undefined8 param_5, undefined8 param_6,
                              undefined8 param_7, undefined8 param_8,
                              char *param_9, undefined4 param_10,
...
double CHUNK__vec_dot_normalized_safe(double param_1, double param_2,
                                      double param_3, double param_4,
                                      double param_5, double param_6);
void ray_scale(float scale, struct Vec3f *ray_start, struct Vec3f *ray_end,
               struct Vec3f *out_ray_end);
...
double mtxa_from_mtxb_translate_xyz(undefined8 param_1, undefined8 param_2,
                                    undefined8 param_3);
...
void CHUNK__mtxa_rigid_inv_tf_vec_xyz(float x, float y, float z,
                                      undefined4 param_4, struct Vec3f *dst,
                                      void *mtxa);
...
void g_math_unk7(double param_1, struct Quat *param_2, float *param_3,
                 float *param_4);
void g_math_unk8(double param_1, struct Quat *param_2, float *param_3,
                 float *param_4);
void mtxa_to_quat(struct Quat *out_quat);
void quat_from_axis_angle(struct Quat *quat, struct Vec3f *axis, int angle);
void g_math_unk9_smth_w_quats(double param_1, struct Quat *param_2,
                              struct Vec3f *param_3);
void quat_to_axis_angle(struct Quat *quat, undefined4 out_axis);
void quat_normalize(struct Quat *quat);
void quat_from_dirs(struct Quat *out_quat, struct Vec3f *start,
                    struct Vec3f *end);
void quat_slerp(float t, struct Quat *dest, struct Quat *quat1,
                struct Quat *quat2);
void g_math_quat_smth2(struct Quat *dst, struct Quat *quat1,
                       struct Quat *quat2);
void g_math_unk10(struct Vec3f *param_1, struct Vec3f *param_2);
void g_math_unk11(struct Vec3f *param_1, struct Vec3f *param_2);
void ray_to_euler(struct Vec3f *param_1, struct Vec3f *param_2,
                  struct Vec3s *param_3);
void ray_to_euler_xy(float *param_1, float *param_2, undefined2 *param_3,
                     undefined2 *param_4);
...
void g_math_unk13(undefined4 param_1, undefined4 param_2,
                  struct Vec3f *param_3);
...
void g_math_unk17(double param_1, double param_2, struct Vec3f *param_3,
                  struct Vec3f *param_4);
void g_math_unk18(double param_1, double param_2, struct Vec3f *param_3,
                  struct Vec3f *param_4);
void g_math_unk19(double param_1, double param_2, struct Vec3f *param_3,
                  struct Vec3f *param_4);
...
void g_init_gx(BOOL32 make_second_fifo, u32 fifo_size,
               int g_something_with_alpha);
...
void g_init_gma_texture(struct GmaTextureDescriptor *tex_desc,
                        struct TplTextureHeader *tpl_texture,
...
s32 dvd_read_with_prio2(struct DVDFileInfo *fileInfo, void *addr, s32 length,
                        s32 offset);
...
void g_set_some_projection_matrix(double fov, double aspect_ratio,
...
void g_some_GmaSomeStruct_func7(struct GmaSomeStruct *gma_struct,
                                GXTevStageID *param_2);
double g_some_GmaSomeStruct_func(struct GmaSomeStruct *param_1);
double g_avdisp_smth_with_tex_materials(struct GmaModelHeader *gma_header,
                                        struct GmaSomeStruct *gma_struct);
void g_some_GmaSomeStruct_func3(struct GmaSomeStruct *gma_struct,
                                int g_some_gma_value);
void g_something_with_GmaSomeStruct_GXBlendFactor(
    struct GmaSomeStruct *gma_struct);
...
int *g_some_GmaSomeStruct_func8(uint *param_1, int param_2,
                                struct GmaSomeStruct *gma_struct, int param_4,
                                struct GmaModelHeader *gma_header);
undefined8 g_avdisp_smth_with_transforms(struct GmaSomeStruct *vert_control);
void g_avdisp_smth_with_skin_effective_model(struct GmaModelHeader *gma_header,
                                             struct GmaSomeStruct *gma_struct,
                                             int param_3);
void g_avdisp_func9(struct GmaModelHeader *param_1,
                    struct GmaSomeStruct *param_2, int param_3, int param_4);
uint *g_avdisp_static_models(struct GmaModelHeader *gma_model,
                             struct GmaSomeStruct *gma_struct,
                             int g_some_gma_field_value);
...
void g_init_gma(struct GmaBuffer *gma_buffer, struct GmaHeader *gma_header,
                struct TplBuffer *tpl);
int g_init_gma_model_materials(struct GmaModelHeader *model,
                               struct TplBuffer *tpl,
                               struct GXTexObj *g_texobj_array);
void g_memcpy_using_locked_cache(void *dest, void *curr_src_1_1_1,
                                 size_t count);
...
void __ull2dec(undefined *param_1, undefined4 param_2, uint param_3,
               uint param_4);
...
int __pformatter(void (*WriteProc)(void *, char *, size_t), void *WriteProcArg,
                 char *format_str, va_list arg, int is_secure);
...
uint __strtoul(uint param_1, uint param_2, undefined *param_3,
...
uint __kernel_rem_pio2(double *param_1, double *param_2, int param_3,
...
void TRKNubMainLoop(undefined4 param_1, undefined4 param_2, undefined4 param_3,
...
void TRKAppendBuffer1_ui64(int param_1, undefined4 param_2, undefined4 param_3,
                           undefined4 param_4);
...
void TRKDoContinue(int param_1, undefined4 param_2, undefined4 param_3,
                   undefined4 param_4, undefined4 param_5, undefined4 param_6,
                   undefined4 param_7, undefined4 param_8);
...
int HandlePositionFileSupportRequest(undefined4 param_1, undefined4 *param_2,
                                     undefined param_3, undefined *param_4);
int HandleCloseFileSupportRequest(undefined4 param_1, undefined *param_2);
int HandleOpenFileSupportRequest(char *param_1, undefined param_2,
                                 undefined4 *param_3, undefined *param_4);
int TRKRequestSend(int param_1, int *param_2, uint param_3, int param_4,
                   int param_5);
int TRKSuppAccessFile(int param_1, int param_2, uint *param_3, char *param_4,
...
undefined4 TRKPPCAccessFPRegister(undefined4 *param_1, uint param_2,
                                  int param_3);
undefined4 TRKTargetStop(void);
...
undefined4 TRKTargetStepOutOfRange(undefined4 param_1, undefined4 param_2,
                                   int param_3);
...
int TRKTargetAccessExtended2(uint param_1, uint param_2, int param_3,
                             int *param_4, int param_5);
uint TRKTargetAccessExtended1(uint param_1, uint param_2, int param_3,
                              int *param_4, int param_5);
int TRKTargetAccessFP(uint param_1, uint param_2, int param_3, int *param_4,
                      int param_5);
undefined4 TRKTargetAccessDefault(int param_1, uint param_2, int param_3,
                                  int *param_4, int param_5);
int TRKTargetAccessMemory(int param_1, uint param_2, int *param_3,
...
void g_twi_0(void);
void g_twi_0(void);
undefined8 InitMetroTRK(undefined4 param_1, undefined4 param_2, int param_3,
                        undefined4 param_4, undefined4 param_5,
...
void TRK_main(undefined4 param_1, undefined4 param_2, undefined4 param_3,
              undefined4 param_4, undefined4 param_5, undefined4 param_6,
              undefined4 param_7, undefined4 param_8);
undefined4 *TRKLoadContext(undefined4 *param_1, int param_2, undefined4 param_3,
                           undefined4 param_4, undefined4 param_5,
                           undefined4 param_6, undefined4 param_7,
                           undefined4 param_8);
void empty_function(void);
...
void TRKEXICallBack(undefined4 param_1, undefined4 *param_2, undefined4 param_3,
                    undefined4 param_4, undefined4 param_5, undefined4 param_6,
                    undefined4 param_7, undefined4 param_8);
undefined4 TRKTargetContinue(undefined4 param_1, undefined4 param_2,
                             undefined4 param_3, undefined4 param_4,
                             undefined4 param_5, undefined4 param_6,
                             undefined4 param_7, undefined4 param_8);
...
undefined4 __write_file(undefined4 param_1, undefined4 param_2,
                        undefined4 *param_3);
undefined4 __write_file(undefined4 param_1, undefined4 param_2,
                        undefined4 *param_3);
undefined4 __TRK_write_console(undefined4 param_1, undefined4 param_2,
                               undefined4 *param_3);
undefined4 __TRK_write_console(undefined4 param_1, undefined4 param_2,
                               undefined4 *param_3);
void empty_function(void);
...
undefined4 return_0(void);
undefined4 return_0(void);
void load_additional_rel(char *rel_filepath,
                         struct RelBufferInfo *rel_buffer_ptrs);
...
void *alloc_from_heap_or_panic(OSHeapHandle heap, u32 size, char *file,
                               int line);
...
void g_draw_3d(undefined8 param_1, undefined8 param_2, double param_3,
               ...
               undefined4 param_16);
void draw_mode(undefined8 param_1, undefined8 param_2, double param_3,
               undefined8 param_4, undefined8 param_5, undefined8 param_6,
               undefined8 param_7, undefined8 param_8, undefined4 param_9,
               undefined4 param_10, undefined4 *param_11, int param_12,
               undefined4 param_13, undefined4 param_14, undefined4 param_15,
...
void take_pausemenu_screenshot(void *out_image_buffer, undefined4 src_left_px,
                               undefined4 src_top_px, short width_px,
                               short height_px, GXTexFmt fmt);
...
void g_some_camera_parent_func(
    ...
    undefined8 param_15, undefined4 param_16, undefined4 param_17);
...
void set_camera_view_settings(float viewport_pos_x, float viewport_pos_y,
                              float viewport_size_x, float viewport_size_y,
                              undefined4 camera_idx);
...
bool g_is_sphere_visible_scaled(double radius, double g_scale,
                                struct Vec3f *center);
...
bool did_ball_enter_goal(struct Ball *ball, int *out_stage_goal_idx,
...
void g_something_with_goals_and_ball(struct Ball *ball, int goal_idx,
                                     int ig_idx, undefined param_4);
...
void g_printf_sound_error(bool is_warning, char *origin,
                          undefined4 g_error_format, ...);
...
undefined4 g_something_calls_sndFXKeyOff(uint param_1, int param_2,
                                         int param_3);
...
void SoftStreamSEReq(char param_1, int sfx_id, undefined4 sfx_volume,
                     undefined4 sfx_pan_L, undefined4 sfx_pan_R);
void empty_function(void);
void call_SoftStreamSEReq_arg_0(int sfx_id, uint sfx_volume, uint sfx_pan_L,
                                uint sfx_pan_R);
void g_stop_music_sound_debug(int param_1);
void g_some_dvd_read_async_sound_callback2(undefined4 param_1,
                                           struct DVDCommandBlock *param_2);
...
void g_another_SoftStreamStart_wrapper_of_some_sort(BgmTrack param_1,
                                                    undefined4 param_2);
undefined4 play_track_and_fade_out_other_tracks(undefined4 param_1,
                                                undefined4 param_2,
                                                byte volume);
undefined4 g_smth_related_to_music(int param_1_00, int param_2_00,
                                   int param_3_00, char param_4, char param_5);
...
void empty_function(void);
uint g_something_with_sound4(undefined *param_1, undefined4 param_2,
                             undefined param_3, undefined param_4,
                             undefined param_5, undefined param_6);
int g_smgr_allocate_stereo(undefined4 param_1, undefined4 param_2,
                           undefined param_3, undefined param_4, char *param_5,
                           char *param_6, char param_7);
void g_some_dvd_read_async_sound_callback(s32 result,
                                          struct DVDFileInfo *file_info);
void event_adx_init(undefined8 param_1, undefined8 param_2, undefined8 param_3,
                    undefined8 param_4, undefined8 param_5, undefined8 param_6,
                    undefined8 param_7, undefined8 param_8, undefined4 param_9,
                    ...
                    undefined4 param_16);
void event_adx_tick(undefined4 param_1, undefined4 param_2, undefined4 param_3,
                    undefined4 param_4, undefined4 param_5, undefined4 param_6,
                    undefined4 param_7, undefined4 param_8);
...
undefined4 g_some_printf_function_1(undefined8 param_1, undefined8 param_2,
                                    undefined8 param_3, undefined8 param_4,
                                    undefined8 param_5, undefined8 param_6,
                                    undefined8 param_7, undefined8 param_8,
                                    char *param_9, undefined4 param_10,
...
undefined4 g_some_printf_function_2(undefined8 param_1, undefined8 param_2,
                                    undefined8 param_3, undefined8 param_4,
                                    undefined8 param_5, undefined8 param_6,
                                    undefined8 param_7, undefined8 param_8,
...
void g_something_with_some_sort_of_lzs(undefined4 param_1, undefined4 param_2,
                                       char *p_lz_path, char *lz_path);
void empty_function(void);
void g_ape_upright(struct Ape *ball);
void g_move_ape_into_ball(struct Ape *ape);
void g_default_ape_draw(struct Ape *ape, undefined4 param_2, undefined4 param_3,
                        undefined4 param_4, undefined4 param_5,
                        undefined4 param_6, undefined4 param_7,
                        undefined4 param_8);
void empty_function(void);
...
void create_stage_player(byte player_index, PhysicsMode physicsmode,
                         byte ape_id, byte color_idx, uint LOD, void *draw_func,
                         int g_variant);
void g_create_stage_player_wrapper(byte player_index, PhysicsMode physicsMode,
                                   byte ape_id, byte param_4, ApeLOD LOD,
                                   void *draw_func);
void challenge_mode_init(void);
void event_ball_tick(void);
void event_ball_dest(void);
void add_bananas(int bananas_to_add);
void g_something_with_translating_items(double param_1, int param_2,
                                        struct Vec3f *param_position);
...
void ball_physics_g_something_w_poastgoal_slowdown_blast_up(
    struct Ball *param_1);
...
void init_physicsball_from_ball(struct Ball *ball,
                                struct PhysicsBall *physicsball);
void g_copy_physicsball_to_ball(struct Ball *ball,
                                struct PhysicsBall *physicsball);
...
undefined4 *g_some_ball_stage_coli_func(struct PhysicsBall *physicsball,
                                        struct StagedefFileHeader *stagedef);
undefined4 get_coli_grid_cell_tris(float x, float z,
                                   struct StagedefColiHeader *coli_header);
void g_something_with_triangle_collision_2(struct PhysicsBall *physicsball,
                                           struct StagedefColiTri *param_2);
undefined8 g_something_with_triangle_collision(struct PhysicsBall *param_1,
                                               struct StagedefColiTri *param_2);
void g_some_jamabar_coli_func(struct PhysicsBall *physicsball, float *param_2);
void g_related_to_cylinder_collision_objs(
    struct PhysicsBall *physball, struct StagedefColiCylinder *cylinder);
void g_related_to_sphere_collision_objs(struct PhysicsBall *param_1,
                                        struct StagedefColiSphere *param_2);
void g_related_to_cone_collision_objs(struct PhysicsBall *param_1,
                                      struct StagedefColiCone *param_2);
void g_something_with_physicsball_restitution(struct PhysicsBall *physicsball,
                                              struct Vec3f *param_2);
BOOL32 does_line_intersect_rect(struct Vec3f *pos1, struct Vec3f *pos2,
                                struct Rect *rect);
void stobj_jamabar_child_coli(struct PhysicsBall *physicsball,
                              struct Stobj *stobj);
void raycast_stage_down(struct Vec3f *origin, struct RaycastHit *out_hit,
                        struct Vec3f *out_vel_at_point);
BOOL32 raycast_tri(struct Vec3f *line_origin, struct Vec3f *line_dir,
                   struct StagedefColiTri *tri);
BOOL32 raycast_cone(struct Vec3f *line_origin, undefined4 line_dir,
                    struct StagedefColiCone *cone, struct Vec3f *out_hit_pos,
                    struct Vec3f *out_hit_normal);
BOOL32 raycast_sphere(struct Vec3f *line_origin, struct Vec3f *line_dir,
                      struct StagedefColiSphere *sphere,
                      struct Vec3f *out_hit_pos, struct Vec3f *out_hit_normal);
BOOL32 raycast_cylinder(undefined4 line_origin, undefined4 line_dir,
                        struct StagedefColiCylinder *cylinder,
                        undefined4 out_hit_pos, undefined4 out_hit_normal);
uint g_related_to_goal_collision(struct PhysicsBall *param_1,
                                 struct StagedefGoal *param_2);
void tf_physicsball_by_mtxa(struct PhysicsBall *physicsball1,
                            struct PhysicsBall *physicsball2);
void inv_tf_physicsball_by_mtxa(struct PhysicsBall *src_physicsball,
                                struct PhysicsBall *dest_physicsball);
void tf_physball_to_itemgroup_space(struct PhysicsBall *physicsball,
                                    int itemgroup_idx);
uint g_something_w_ig_and_coli_headers(
    struct Itemgroup *ig_list, struct StagedefColiHeader *coli_header_list,
    undefined4 param_3, struct Vec3f *physicsball_x);
undefined4
g_something_w_ig_and_coli_headers_2(struct Itemgroup *ig_list,
                                    struct StagedefColiHeader *coli_header_list,
                                    struct Vec3f *physicsball_pos);
...
double g_advance_itemgroup_anim_frame(struct Itemgroup *itemgroup,
                                      struct StagedefColiHeader *colis_header);
...
char *g_something_with_strings_and_dvd_file_reading(int param_1, char *param_2,
                                                    int param_3);
...
void g_advance_replay(double replay_frames_remaining, int param_2,
                      struct Vec3f *position_out);
void g_smth_with_seesaws_and_replays(int param_1, uint param_2, uint param_3,
                                     void *param_4);
...
void overwrite_effect_funcs(EffectType effect_type,
                            void (**new_effect_funcs)(struct Effect *));
...
void g_render_foreground_objects(Mtx *g_stage_tilt_mtx,
                                 struct StagedefForegroundModel *fg_model,
                                 int foreground_model_count);
...
void bg_futsal_tick(undefined4 param_1, undefined4 param_2, undefined4 param_3,
                    undefined4 param_4, undefined4 param_5, undefined4 param_6,
                    undefined4 param_7, undefined4 param_8);
...
void g_init_stagedef_bg_fg_models(struct StagedefBackgroundModel *model,
                                  int count,
                                  struct GSomeBgStruct *some_bg_struct,
                                  undefined *param_4);
...
void bg_bil2_tick(undefined4 param_1, undefined4 param_2, undefined4 param_3,
                  undefined4 param_4, undefined4 param_5, undefined4 param_6,
                  undefined4 param_7, undefined4 param_8);
...
u32 g_update_cm_course(Difficulty difficulty, s32 course_stage_num,
                       ModeFlag mode_flags);
int calc_course_idx(Difficulty difficulty, ModeFlag mode_flags);
uint g_are_on_final_course_level(int difficulty_id, int course_stage,
                                 uint difficulty_flags);
undefined4 is_bonus_stage_being_played(int param_1);
void g_something_with_cm_entries_practice_mode(void);
undefined4 g_smth_with_cm_entries_in_main_menu(int param_1, int param_2,
                                               uint param_3);
void empty_function(void);
void update_cm_unlocked_levels(Difficulty difficulty, int param_2,
                               ModeFlag mode_flags);
void clear_cm_player_progress(void);
void update_course_progress(void);
void g_something_with_cm_player_progress(void);
void sprite_debug_course_display_disp(undefined8 param_1, undefined8 param_2,
                                      double param_3, double param_4,
                                      undefined8 param_5, undefined8 param_6,
                                      undefined8 param_7, undefined8 param_8,
...
void init_coli_header_items(struct StagedefColiHeader *coli_header,
                            int coli_header_count);
...
uint g_some_item_collision_check(double ball_scale, double item_scale,
                                 struct Vec3f *ball_prev_pos,
                                 struct Vec3f *ball_pos,
                                 struct Vec3f *item_pos_copy,
                                 struct Vec3f *item_pos);
void g_something_with_item_coli(double param_1, double param_2,
                                struct Vec3f *param_3, struct Vec3f *param_4,
                                struct Vec3f *param_5);
...
void load_bumper_stobjs(struct StagedefColiHeader *coli_header_list,
                        int coli_header_count);
void load_jamabar_stobjs(struct StagedefColiHeader *coli_header_list,
                         int coli_header_count);
void load_button_stobjs(struct StagedefColiHeader *coli_header_list,
                        int coli_header_count);
...
void stobj_bumper_bgspecial_coli(struct Stobj *stobj,
                                 struct PhysicsBall *physicsball);
...
void load_goaltape_and_goalbag_stobjs(
    struct StagedefColiHeader *coli_header_list, int coli_header_count);
...
void stobj_goalbag_exmaster_coli(struct Stobj *stobj,
                                 struct PhysicsBall *physicsball);
void stobj_goalbag_exmaster_dest(struct Stobj *stobj);
void stobj_goalbag_exmaster_cb_f(struct Stobj *stobj);
void g_something_with_goals_and_physicsball(int goal_idx,
                                            struct PhysicsBall *physicsball);
float g_get_sphere_camera_zdist_clamped(float radius, struct Vec3f *origin);
void load_returngate_stobjs(struct StagedefColiHeader *coli_header_list,
                            int coli_header_count);
...
void stobj_returngate_coli(struct Stobj *stobj,
                           struct PhysicsBall *physicsball);
...
void g_printf_draw_with_font(double param_1, double param_2, double param_3,
                             double param_4, double param_5, double param_6,
                             double param_7, double param_8, char *g_format,
                             undefined4 param_10, undefined4 param_11,
                             undefined4 param_12, undefined4 param_13,
                             undefined4 param_14, undefined4 param_15,
                             undefined4 param_16);
void g_some_printf_function_3(double param_1, undefined8 param_2,
                              undefined8 param_3, undefined8 param_4,
                              undefined8 param_5, undefined8 param_6,
                              undefined8 param_7, undefined8 param_8,
                              char *param_9, undefined4 param_10,
...
int g_get_tex_id(undefined4 param_1, ushort param_2, ushort *param_3,
                 int param_4);
void g_smth_with_font_drawing(byte *param_1);
double g_something_with_fonts4(int param_1, int param_2, char param_3);
void g_some_printf_function_4(undefined8 param_1, undefined8 param_2,
                              undefined8 param_3, undefined8 param_4,
                              undefined8 param_5, undefined8 param_6,
                              undefined8 param_7, undefined8 param_8,
                              int param_9, char *param_10, undefined4 param_11,
                              undefined4 param_12, undefined4 param_13,
                              undefined4 param_14, undefined4 param_15,
                              undefined4 param_16);
...
void create_game_over_letter_sprite(s32 param_1, uint x, uint y,
                                    undefined4 param_4, char *letter);
...
void create_postgoal_score_sprites(undefined4 param_1, undefined4 param_2,
                                   uint param_3);
...
void empty_function(void);
void empty_function(void);
void g_ape_anim_head(struct Ape *ape, struct Vec3f *ape_facedir_point,
                     int flags);
void empty_function(void);
...
u32 g_something_with_reading_dvd_file(int *dvdEntrynum, void *buffer,
                                      u32 length, int offset);
...
void g_set_minigame_specific_funcs(undefined4 param_1, undefined4 param_2,
                                   undefined4 param_3, undefined4 param_4);
...
void event_commend_tick(double param_1, undefined4 param_2, undefined4 param_3,
                        undefined4 param_4, undefined4 param_5,
                        undefined4 param_6, undefined4 param_7,
...
void g_some_printf_function_5(undefined8 param_1, undefined8 param_2,
                              undefined8 param_3, undefined8 param_4,
                              undefined8 param_5, undefined8 param_6,
                              undefined8 param_7, undefined8 param_8,
                              char *param_9, undefined4 param_10,
                              ...
                              undefined4 param_13, undefined4 param_14,
                              undefined4 param_15, undefined4 param_16);
void g_something_with_card11(undefined8 param_1, undefined8 param_2,
                             undefined8 param_3, undefined8 param_4,
                             undefined8 param_5, undefined8 param_6,
                             undefined8 param_7, undefined8 param_8);
void g_something_with_card9(undefined8 param_1, undefined8 param_2,
                            undefined8 param_3, undefined8 param_4,
                            undefined8 param_5, undefined8 param_6,
                            undefined8 param_7, undefined8 param_8);
void g_something_with_card5(undefined8 param_1, undefined8 param_2,
                            undefined8 param_3, undefined8 param_4,
                            undefined8 param_5, undefined8 param_6,
                            undefined8 param_7, undefined8 param_8);
void g_something_with_card4(undefined8 param_1, undefined8 param_2,
                            undefined8 param_3, undefined8 param_4,
                            undefined8 param_5, undefined8 param_6,
                            undefined8 param_7, undefined8 param_8);
void g_something_with_card8(void);
void g_something_with_card6(undefined8 param_1, undefined8 param_2,
                            undefined8 param_3, undefined8 param_4,
                            undefined8 param_5, undefined8 param_6,
                            undefined8 param_7, undefined8 param_8);
void g_something_with_card2(int card_chan, int param_2);
uint g_something_with_fonts(void);
void g_some_printf_function_6(undefined8 param_1, undefined8 param_2,
                              undefined8 param_3, undefined8 param_4,
                              undefined8 param_5, undefined8 param_6,
                              undefined8 param_7, undefined8 param_8,
                              int param_9, int param_10, int param_11,
                              char *param_12, undefined4 param_13,
                              undefined4 param_14, undefined4 param_15,
                              undefined4 param_16);
void smd_mini_ranking_init(void);
void smd_mini_ranking_tick(void);
void g_load_preview_texture(struct SpriteTex *sprite_tex, char *file_path,
                            undefined4 param_3, u16 width, u16 height,
                            GXTexFmt format);
...
void g_something_with_name_entry(undefined8 param_1, undefined8 param_2,
                                 undefined8 param_3, undefined8 param_4,
                                 undefined8 param_5, undefined8 param_6,
                                 undefined8 param_7, undefined8 param_8);
...
void stobj_nameent_btn_coli(struct Stobj *stobj,
                            struct PhysicsBall *physicsball);
...
void g_stores_doubles(double param_1, double param_2, double param_3,
                      double param_4);
void g_stores_doubles2(double param_1, double param_2, double param_3,
                       double param_4);
...
void empty_function(void);
void g_load_skl(struct SKLRoot *sklRoot, struct SKLInfo *boneData,
                struct SKLFile *sklFile);
...
void gan_setanim_edance(undefined4 param_1, undefined param_2, uint param_3,
...
void g_something_with_new_ape(struct Ape *ape, int chara_index,
                              int scene_index);
...
void g_ape_anim_body(struct Ape *ape, undefined4 param_2, undefined4 param_3,
                     undefined4 param_4, undefined4 param_5, undefined4 param_6,
                     undefined4 param_7, undefined4 param_8);
void g_load_mal_files_from_disc(int chara_index, int LOD, int s_mal,
                                int game_index);
...
int g_get_ape_flag(struct Ape *ape, undefined4 g_something_with_game,
                   int param_3);
void gan_setanim_e4(struct Ape *ape);
void gan_setAnim2(struct Ape *ape, int param_2, short *param_3);
int g_some_ape_anim_func2(int param_1, int param_2, undefined4 param_3,
                          undefined4 param_4, int param_5);
...
void sprite_rank_disp(undefined8 param_1, undefined8 param_2,
                      undefined8 param_3, undefined8 param_4,
                      undefined8 param_5, undefined8 param_6,
                      undefined8 param_7, undefined8 param_8);
...
void g_some_set_ape_anim(struct Ape *ape, undefined chara_anim_type,
                         undefined2 param_3);
...
void g_something_to_do_with_soft_fx(undefined4 param_1, undefined4 param_2,
                                    int param_3);
void empty_function(void);
void empty_function(void);
void empty_function(void);
...
void smd_author_play_tick(undefined4 param_1, undefined4 param_2,
                          undefined4 param_3, undefined4 param_4,
                          undefined4 param_5, undefined4 param_6,
                          undefined4 param_7, undefined4 param_8);
...
void g_assign_new_main_and_sub_mode_for_play_tick(MainMode param_1,
                                                  SubMode param_2);
...
void g_display_playpoint_or_gift_message(double param_1, double param_2,
                                         undefined param_3);
...
void smd_game_roll_tick(undefined4 param_1, undefined4 param_2_00,
                        short param_3);
...
undefined4 *g_check_if_game_over_or_after_credits_sequence(int param_1,
                                                           undefined4 *param_2);
void empty_function(void);
...
void sel_ngc_prolog(undefined8 param_1, undefined8 param_2, undefined8 param_3,
                    undefined8 param_4, undefined8 param_5, undefined8 param_6,
                    undefined8 param_7, undefined8 param_8);
...
void smd_sel_ngc_init(undefined8 param_1, undefined8 param_2,
                      undefined8 param_3, undefined8 param_4,
                      undefined8 param_5, undefined8 param_6,
                      undefined8 param_7, undefined8 param_8);
...
void g_menu_main_handler(undefined8 param_1, undefined8 param_2,
                         undefined8 param_3, undefined8 param_4,
                         undefined8 param_5, undefined8 param_6,
                         undefined8 param_7, undefined8 param_8);
void g_menu_party_mode_handler(undefined8 param_1, undefined8 param_2,
                               undefined8 param_3, undefined8 param_4,
                               undefined8 param_5, undefined8 param_6,
                               undefined8 param_7, undefined8 param_8);
void switch_mode_to_smd_adv_title_reinit(void);
void g_something_with_menus2(undefined8 param_1, undefined8 param_2,
                             undefined8 param_3, undefined8 param_4,
                             undefined8 param_5, undefined8 param_6,
                             undefined8 param_7, undefined8 param_8,
                             undefined4 param_9, undefined4 param_10,
                             int *param_11, MenuScreenID *param_12,
                             u8 *param_13, undefined4 param_14,
                             undefined4 param_15, undefined4 param_16);
...
void sprite_practice_stage_select_disp(
    undefined8 param_1, undefined8 param_2, undefined8 param_3,
    undefined8 param_4, undefined8 param_5, undefined8 param_6,
    undefined8 param_7, undefined8 param_8, int param_9, undefined4 param_10,
...
void g_bowl_drawFunc(struct Ape *param_1, int param_2, undefined4 param_3,
                     undefined4 param_4, undefined4 param_5, undefined4 param_6,
                     undefined4 param_7, undefined4 param_8);
...
void g_draw_debug_mode_sound_screen(undefined8 param_1, undefined8 param_2,
                                    undefined8 param_3, undefined8 param_4,
                                    undefined8 param_5, undefined8 param_6,
                                    undefined8 param_7, undefined8 param_8,
                                    undefined4 param_9, undefined4 param_10,
...
void empty_function(void);
void empty_function(void);
void g_some_printf_function_7(undefined8 param_1, undefined8 param_2,
                              undefined8 param_3, undefined8 param_4,
                              undefined8 param_5, undefined8 param_6,
                              undefined8 param_7, undefined8 param_8,
...
void gan_setanim_e8(undefined4 param_1, undefined4 param_2, undefined4 param_3,
                    undefined4 param_4, int param_5);
...
void empty_function(void);
void g_some_printf_function_8(undefined8 param_1, undefined8 param_2,
                              undefined8 param_3, undefined8 param_4,
                              undefined8 param_5, undefined8 param_6,
                              undefined8 param_7, undefined8 param_8,
                              char *param_9, undefined4 param_10,
                              ...
                              undefined4 param_13, undefined4 param_14,
                              undefined4 param_15, undefined4 param_16);
void g_some_printf_function_9(double param_1, double param_2,
                              undefined8 param_3, undefined8 param_4,
                              undefined8 param_5, undefined8 param_6,
                              undefined8 param_7, undefined8 param_8,
                              uint param_9, undefined4 param_10, char *param_11,
                              undefined4 param_12, undefined4 param_13,
                              undefined4 param_14, undefined4 param_15,
                              undefined4 param_16);
...
void g_read_something_for_shooting_from_dvd(char *param_1, uint **param_2,
                                            int *param_3);
void g_read_something_for_shooting_2(ushort **param_1, char *param_2,
                                     struct Ape **param_3);
...
void g_some_printf_function_10(undefined8 param_1, undefined8 param_2,
                               undefined8 param_3, undefined8 param_4,
                               undefined8 param_5, undefined8 param_6,
                               undefined8 param_7, undefined8 param_8,
                               char *param_9, undefined4 param_10,
...
void empty_function(void);
void empty_function(void);
void g_load_baseball(byte player_id, PhysicsMode param_2, undefined param_3,
                     byte param_4, byte param_5, uint param_6, void *param_7,
                     int param_8);
void empty_function(void);
void empty_function(void);
void empty_function(void);
